...
const ImportTransactionsModal = ({ onClose, onSuccess }: ImportTransactionsModalProps) => {
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      console.log(`📤 [FRONTEND] Enviando ${transactions.length} transações para o backend... (país: ${country})`);
      const response = await transactionApi.importTransactions({ transactions, country });
...
13/10/2025,PAGAMENTO DE BOLETO OUTROS BANCOS  CAROL,,"-410,00","2.565,05"`;
    ...
    a.href = url;
    a.download = 'modelo_exemplo_transacoes.csv';
    a.click();
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-gray-900/80 via-gray-800/80 to-blue-900/80 backdrop-blur-sm flex items-center justify-center sm:justify-center z-50 pl-16 pr-2 py-2 sm:p-4 animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[95vh] sm:max-h-[85vh] flex flex-col shadow-2xl transform transition-all animate-slide-up my-auto ml-auto sm:ml-0 sm:mx-auto">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gradient-to-b from-gray-50 to-white">
          ...
          {/* Seletor de País - define a base de categorização automática */}
          <div className="mb-4 sm:mb-6">
            <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
              País das transações
              <span className="text-xs font-normal text-gray-500 ml-1">(define a categorização automática)</span>
            </label>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setCountry('BR')}
                className={`flex-1 px-3 py-2.5 rounded-xl border-2 font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                  country === 'BR'
              ...
              >
                <span className="text-lg">🇩🇪</span> Alemanha
              </button>
            </div>
          </div>
