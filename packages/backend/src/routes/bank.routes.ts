...
router.get('/accounts/:accountId/diagnose', async (req: Request, res: Response) => {
  try {
    ...
    const supabaseIds = new Set((supabaseTransactions || []).map(t => t.transaction_id));

    const missingInSupabase = pluggyTransactions.filter(t => !supabaseIds.has(t.transaction_id));
    const extraInSupabase = (supabaseTransactions || []).filter(t => !pluggyIds.has(t.transaction_id));
